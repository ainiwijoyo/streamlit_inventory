...
def get_data_transaksi():
    ...
    WHERE t.jenis_transaksi = 'keluar' AND t.status = 'selesai'
    """
...
def tampilkan_barang_keluar():
    ...
    with tambah_barang_keluar_popover:
        with st.form("form_tambah_barang"):
            ...
            pilihan_barang = {f"{barang[1]}": barang[0]
            ...
            pilihan_ruangan = {ruangan[1]: ruangan[0]
                               for ruangan in data_ruangan}
            ruangan_terpilih = st.selectbox(
                "Ruangan", list(pilihan_ruangan.keys()))
            id_ruangan = pilihan_ruangan[ruangan_terpilih]
