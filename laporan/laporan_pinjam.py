...
def ambil_data(tgl_dari, tgl_sampai):
    ...
    SELECT tb_transaksi.tanggal, tb_barang.nama_barang, tb_merek.nama_merek, tb_ruangan.nama_ruangan, 
           tb_kondisi.nama_kondisi, tb_transaksi.status, tb_transaksi.jumlah
    FROM tb_transaksi
...
class PDF(FPDF):
    ...
    def laporan(self, data, tgl_dari, tgl_sampai):
        ...
        for i, row in enumerate(data):
            ...
            status = 'DIPINJAM' if row[5] == 'belum' else 'DIKEMBALIKAN'
            self.set_x(table_start_x)
            ...
            self.cell(30, 10, row[4], 1, 0, 'C')
            self.cell(30, 10, status, 1, 0, 'C', fill=row[5] == 'belum')
            self.cell(20, 10, str(row[6]), 1, 1, 'C')
            total_barang += row[6]

        self.set_font('Arial', 'B', 10)
        self.set_x(table_start_x)
        self.cell(190, 10, 'TOTAL SEMUA PEMINJAMAN', 1, 0, 'C')
