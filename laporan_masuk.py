...
def ambil_data(tgl_dari, tgl_sampai):
    ...
    WHERE tb_transaksi.jenis_transaksi = 'masuk' AND tb_transaksi.tanggal BETWEEN %s AND %s
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 2, 'LAPORAN BARANG MASUK BAGIAN TIK', 0, 1, 'C')
    ...
    def laporan(self, data, tgl_dari, tgl_sampai):
        ...
        self.set_font('Arial', 'B', 10)
        self.cell(160, 10, 'TOTAL SEMUA BARANG MASUK', 1, 0, 'C')
...
def laporan_masuk():
    ...
    if st.button('Cetak PDF'):
        ...
        if data:
            ...
            href = f'<a href="data:application/pdf;base64,{b64_pdf}" download="Laporan_Barang_Masuk_{tgl_dari.strftime("%Y-%m-%d")}_{tgl_sampai.strftime("%Y-%m-%d")}.pdf">Unduh Laporan</a>'
            st.markdown(href, unsafe_allow_html=True)
        else:
            st.warning('Tidak ada data transaksi masuk pada periode ini.')
